...
if ! tmux has-session -t dijon 2>/dev/null; then
  tw new-session -d -s dijon -n golf-4 \
     "$CONFIG_DIR/ssh_loop.sh dijon-golf-4.dijonsystems.com" || true
fi
...
tw new-window -t dijon -n deploy \
  "$CONFIG_DIR/ssh_loop.sh deployhost.golf.dijon 'cd /home/dijon/yegor/release-golf-system && bash -l'" || true

# 4) inspect (ssh, cd, activate venv)
tw new-window -t dijon -n inspect \
  "$CONFIG_DIR/ssh_loop.sh deployhost.golf.dijon 'cd /home/dijon/yegor/inspect-golf-system && source venv/bin/activate && exec bash -l'" || true
